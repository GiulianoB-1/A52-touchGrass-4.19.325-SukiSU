...
EXPECTED_TEST_SHA = "e34184f53363f6bb873c2fe0ce1a08ed7d16e94a"
EXPECTED_PRIOR_BAD_SHA = "c5c62f4c936407fb734cae64700f20b68273b059"
# Phase98 intentionally keeps boot diagnostics out of the bisect delta.
...
def main() -> None:
    ...
    if len(commits) < TEST_POSITION:
        raise SystemExit(f"unexpected 4.19.210 range length: {len(commits)}")

    test_sha = commits[TEST_POSITION - 1]
    prior_bad_sha = commits[PRIOR_BAD_POSITION - 1]
    if test_sha != EXPECTED_TEST_SHA:
        raise SystemExit(f"full210 SHA mismatch: {test_sha}")
    if prior_bad_sha != EXPECTED_PRIOR_BAD_SHA:
        raise SystemExit(f"full210 SHA mismatch: {prior_bad_sha}")
    ...
    status = ARTIFACTS / "phase98-full210-name-status.zlist"
    ...
    conflicts = ARTIFACTS / "phase98-full210-conflicts.txt"
    report = ARTIFACTS / "phase98-full210-policy.txt"
    policy_log = ARTIFACTS / "phase98-full210-policy.tsv"
    run(
    ...
    state = (
        "experiment=phase98-current-stack-419210-full\n"
    ...
    (ARTIFACTS / "phase98-full210-state.txt").write_text(state)
    print(state, end="")

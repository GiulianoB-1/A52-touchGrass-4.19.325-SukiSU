 ...
 * RAMOOPS zones. The in-memory copy only retries events emitted before RAMOOPS
...
static atomic_t a52_usr2_backend_seen = ATOMIC_INIT(0);
...
static void a52_usr2_flush_missing(u64 through)
{
	...
	if (through > A52_USR2_CAPACITY)
		through = A52_USR2_CAPACITY;
	for (cursor = 1; cursor <= through; cursor++) {
...
void a52_ackfr_record(const char *fmt, ...)
{
	...
	if (written && atomic_cmpxchg(&a52_usr2_backend_seen, 0, 1) == 0) {
		a52_usr2_write_control("BOOT_BEGIN");
		a52_usr2_flush_missing(seq);
	}
}
...
'''.replace(r"\t", "\t")
        text = text.replace(anchor, writer, 1)

    old = '''\tif (record->type == PSTORE_TYPE_CONSOLE) {
\t\tif (!cxt->cprz)
\t\t\treturn -ENOMEM;
...
\t\treturn 0;
'''
    new = '''\tif (record->type == PSTORE_TYPE_CONSOLE) {
...
def ensure_include(text: str) -> str:
    ...
    if include in text:
        return text
    for anchor in ('#include <linux/module.h>\n', '#include <linux/kernel.h>\n'):
        if anchor in text:
            return text.replace(anchor, anchor + include, 1)
    return include + text


def inject_entry(trace, text: str, domain: str, name: str,
                 suffix: str = "", args: str = "") -> tuple[str, str]:
    found = trace.function_definition(text, name)
    if found is None:
        return text, "missing"
    _, opening, _, _ = found
    marker = f"{domain} A52USR2 enter fn={name}"
    if marker in text[opening:opening + 1800]:
        return text, "already-present"
    fmt = marker + (f" {suffix}" if suffix else "")
...
def patch_functions(trace, root: Path, relative: Path, domain: str,
                    specs: tuple[tuple[str, str, str], ...]) -> dict[str, object]:
    path = root / relative
    if not path.is_file():
        return {"source": str(relative), "missing_file": True, "functions": []}
    text = ensure_include(read(path))
    staged = []
    for name, suffix, args in specs:
        text, state = inject_entry(trace, text, domain, name, suffix, args)
        exits = 0
        if state != "missing":
...
def stage_recorder(root: Path) -> dict[str, object]:
    ...
    checks = {
        ...
        "real_newline_delimiter": 'commit=%08x\\n"' in source and 'commit=%08x\\\\n"' not in source,
...
def self_test() -> None:
    with tempfile.TemporaryDirectory() as tmp:
        ...
        if not first["checks"]["direct_writer_exported"]:
            raise SystemExit("ramoops writer self-test failed")
        if not second["checks"]["console_bank_reserved"]:
            raise SystemExit("ramoops idempotence self-test failed")
...
def main() -> int:
    ...
    missing = [str(item) for item in (HEADER_REL, SOURCE_REL, RAMOOPS_REL)
    ...
    boundaries = {
        "qsee_init": patch_functions(trace, root, Path("drivers/a52_secure/qseecom.c"), "QSEEINIT", (
            ("qseecom_probe", "stage=probe", ""),
            ("qseecom_init_dev", "stage=dev", ""),
            ("qseecom_init_control", "stage=control", ""),
            ("qseecom_parse_dt", "stage=parse_dt", ""),
            ("qseecom_retrieve_ce_data", "stage=ce_data", ""),
            ("qseecom_init_clk", "stage=clock", ""),
            ("qseecom_init_bus", "stage=bus", ""),
            ("qseecom_send_app_region", "stage=app_region", ""),
            ("qseecom_create_kthreads", "stage=kthreads", ""),
            ("qseecom_register_shmbridge", "stage=register_bridges", ""),
        )),
        "qsee_memory": patch_functions(trace, root, Path("drivers/a52_secure/qseecom.c"), "DMABUF", (
            ("qseecom_create_bridge_for_secbuf", "fd=%d", "ion_fd"),
            ("qseecom_dmabuf_map", "fd=%d", "ion_fd"),
            ("qseecom_dmabuf_unmap", "stage=unmap", ""),
            ("qseecom_vaddr_map", "fd=%d", "ion_fd"),
            ("qseecom_vaddr_unmap", "stage=vaddr_unmap", ""),
            ("qseecom_dmabuf_cache_operations", "cache_op=%d", "cache_op"),
            ("__qseecom_set_sb_memory", "stage=listener_sb", ""),
        )),
        "qsee_scm": patch_functions(trace, root, Path("drivers/a52_secure/qseecom.c"), "SCM", (
        ...
        "shmbridge": patch_functions(trace, root, Path("drivers/a52_secure/qtee_shmbridge.c"), "SHMBRIDGE", (
            ...
            ("qtee_shmbridge_free_shm", "stage=free", ""),
            ("qtee_shmbridge_init", "stage=init", ""),
        )),
        "secure_buffer": patch_functions(trace, root, Path("drivers/a52_secure/secure_buffer.c"), "SECUREBUF", (

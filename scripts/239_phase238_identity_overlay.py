#!/usr/bin/env python3
"""Phase 239 runtime identity over the Phase 238 recorder."""
...
NEW = "BOOT rs=ready phase=239 focus=gpu-cx-vdd-parent roots=%u copies=3 crc=crc32c"
MARKER = "A52_PHASE239_GPU_CX_VDD_PARENT_IDENTITY_V1"
...
def patch(text: str, label: str) -> str:
    if NEW in text and MARKER in text:
        return text
    if text.count(OLD) != 1:
        raise RuntimeError(f"{label}: expected exactly one Phase 238 boot identity")
    text = text.replace(OLD, NEW, 1)
    anchor = "A52_PHASE238_BROAD_GPU_SUPPLIER_RECORDER_V1"
    if text.count(anchor) != 1:
        raise RuntimeError(f"{label}: Phase 238 recorder marker count drifted")
...
def locate_generated(args: list[str], cwd: Path | None = None) -> Path:
    ...
    if len(unique) != 1:
        ...
        raise RuntimeError(f"expected one generated Phase 238 recorder root, found {rendered}")
    return unique[0]


def self_test() -> None:
    fixture = "/* A52_PHASE238_BROAD_GPU_SUPPLIER_RECORDER_V1 */\n" + OLD + "\n"
    out = patch(fixture, "fixture")
    if NEW not in out or MARKER not in out or OLD in out:
        raise AssertionError("Phase 239 identity replacement failed")
    if patch(out, "fixture/idempotent") != out:
        raise AssertionError("Phase 239 identity patch is not idempotent")
    with tempfile.TemporaryDirectory() as temp:
        ...
        if locate_generated([], cwd=repo).resolve() != root.resolve():
            raise AssertionError("Phase 239 identity locator failed")
...
def main() -> int:
    ...
    print("Phase 239 runtime identity applied", flush=True)
    return 0

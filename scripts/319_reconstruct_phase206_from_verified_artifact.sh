...
for f in \
  ...
  "$STAGE/recorder-after-phase206.c"; do test -s "$f"; done
...
# Fail closed unless the expired complete source artifact is reproduced exactly.
bash scripts/319_regenerate_phase175_base.sh "$P175"

# Historical 199_ci.sh applied the complete patch to a clean tree. Reset here to
# discard intent-to-add state created while regenerating the patch.
git -C "$ROOT" reset --hard "$GKI_COMMON_SHA"
git -C "$ROOT" clean -fd
git -C "$ROOT" apply --check "$P175"
git -C "$ROOT" apply "$P175"

P177="$PWD/workspace/phase177.patch"; P177_TMP="$PWD/workspace/phase177-payload"
...
git -C "$ROOT" apply --check "$P177"; git -C "$ROOT" apply "$P177"
...
git -C "$ROOT" apply --check "$P179"; git -C "$ROOT" apply "$P179"
...
python3 scripts/197_apply_triple_rs.py --self-test; python3 scripts/197_apply_triple_rs.py --root "$ROOT"
python3 scripts/198_apply_catalog_trace.py --self-test; python3 scripts/198_apply_catalog_trace.py --root "$ROOT"
git -C "$ROOT" diff --check

mkdir -p "$BUILD"; cp "$BASE/config/before-phase199.config" "$BUILD/.config"
...
echo 'Phase319 repair: exact Phase175 -> Phase199 historical replay PASS'

cmp_stage() { cmp "$ROOT/$1" "$STAGE/$2"; }

python3 scripts/200_apply_smmu_defer_trace.py --root "$ROOT"; python3 scripts/200_apply_smmu_defer_trace.py --root "$ROOT"
...
python3 scripts/201_apply_smmu_component_dependency.py --root "$ROOT"; python3 scripts/201_apply_smmu_component_dependency.py --root "$ROOT"
...
python3 scripts/202_apply_driver_core_trace.py --root "$ROOT"; python3 scripts/202_apply_driver_core_trace.py --root "$ROOT"
...
python3 scripts/203_apply_apps_smmu_qsmmuv500_compat.py --root "$ROOT"; python3 scripts/203_apply_apps_smmu_qsmmuv500_compat.py --root "$ROOT"
...
python3 scripts/204_apply_apps_smmu_scm_handoff.py --root "$ROOT"; python3 scripts/204_apply_apps_smmu_scm_handoff.py --root "$ROOT"
...
python3 scripts/206_apply_smmu_display_contracts.py --root "$ROOT"; python3 scripts/206_apply_smmu_display_contracts.py --root "$ROOT"
...
p=Path('scripts/213_apply_ion_transaction_trace.py'); t=p.read_text()
old=r'pr_warn_once("%s: ioctl validate failed\n", __func__);'
new=r'pr_warn_once("%s: ioctl validate failed\\n", __func__);'
if t.count(old)==1 and t.count(new)==0: p.write_text(t.replace(old,new,1))
elif not (t.count(old)==0 and t.count(new)==1): raise SystemExit('Phase213 escape repair unexpected state')
t=p.read_text()
if t.count(new)!=1 or old in t: raise SystemExit('Phase213 escape repair verification failed')

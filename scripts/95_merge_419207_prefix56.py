...
STABLE = WORKSPACE / "linux-stable-4.19.207-prefix56-current"
BASE_TREE = WORKSPACE / "linux-base-4.19.206-prefix56-current"
THEIRS_TREE = WORKSPACE / "linux-theirs-4.19.207-prefix56-current"
...
EXPECTED_TEST_SHA = "6812cb0d4379a55677337f115318b6857a4b905e"
EXPECTED_PRIOR_BAD_SHA = "c5c62f4c936407fb734cae64700f20b68273b059"
# Phase95 intentionally keeps boot diagnostics out of the bisect delta.
...
def main() -> None:
    ...
    if test_sha != EXPECTED_TEST_SHA:
        raise SystemExit(f"prefix56 SHA mismatch: {test_sha}")
    ...
    (ARTIFACTS / "phase95-linux-4.19.207-commit-list.tsv").write_text(commit_list + "\n")

    archive(STABLE, GOOD_TAG, BASE_TREE)
    archive(STABLE, test_sha, THEIRS_TREE)

    status = ARTIFACTS / "phase95-prefix56-name-status.zlist"
    ...
    conflicts = ARTIFACTS / "phase95-prefix56-conflicts.txt"
    report = ARTIFACTS / "phase95-prefix56-policy.txt"
    policy_log = ARTIFACTS / "phase95-prefix56-policy.tsv"
    run(
    ...
    generated_fix = ROOT / "scripts" / ".phase95-prefix56-compile-fix.sh"
    ...
    state = (
        "experiment=phase95-current-stack-419207-prefix56\n"
    ...
    (ARTIFACTS / "phase95-prefix56-state.txt").write_text(state)
    print(state, end="")

...
TRACE_LOG="$LOG_DIR/v5-wrapper-trace.log"
STATUS_FILE="$ARTIFACTS_DIR/v5-wrapper.status"

mkdir -p "$ARTIFACTS_DIR" "$LOG_DIR"

[ -f "$TARGET" ] || {
  echo "Missing v5 wrapper: $TARGET" | tee "$TRACE_LOG" >&2
...
# Preserve the exact generated wrapper/build scripts for audit or failure triage.
for file in \
  "$SCRIPT_DIR/23_build_ack_6_1_logdump_probe_v3.sh" \
  "$SCRIPT_DIR/29_run_ack_6_1_logdump_probe_v5.sh" \

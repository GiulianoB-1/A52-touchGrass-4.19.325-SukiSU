...
STABLE = WORKSPACE / "linux-stable-4.19.207-commit-bisect"
BASE_TREE = WORKSPACE / "linux-base-4.19.206-commit-bisect"
THEIRS_TREE = WORKSPACE / "linux-theirs-4.19.207-midpoint"
GOOD_TAG = "v4.19.206"
BAD_TAG = "v4.19.207"
...
def main() -> None:
    ...
    midpoint_position = max(1, count // 2)
    midpoint_sha = commits[midpoint_position - 1]
    midpoint_subject = run(
        "git", "show", "-s", "--format=%s", midpoint_sha, cwd=STABLE, capture=True
    )
    ...
    archive(STABLE, midpoint_sha, THEIRS_TREE)

    status = ARTIFACTS / "linux-4.19.207-midpoint-name-status.zlist"
    ...
    conflict_list = ARTIFACTS / "linux-4.19.207-midpoint-conflicts.txt"
    policy_report = ARTIFACTS / "linux-4.19.207-midpoint-policy.txt"
    policy_log = ARTIFACTS / "linux-4.19.207-midpoint-policy.tsv"
    run(
        "python3",
        str(POLICY),
        str(KERNEL),
    ...
    generated_fix = ROOT / "scripts/checkpoint_fix_linux_4.19.207-midpoint.generated.sh"
    ...
    generated_build = ROOT / "scripts/build_a52_linux_4.19.207_midpoint.generated.sh"
    ...
    build_text = build_text.replace(
        ...
        "a52xq-linux-4.19.207-commit-midpoint-no-root-",
        1,
    )
    build_text = build_text.replace(
    ...
    state = (
        ...
        f"known_good_applied_commits=0\n"
        f"known_bad_tag={BAD_TAG}\n"
        f"known_bad_commit={bad_sha}\n"
        f"known_bad_applied_commits={count}\n"
        f"test_commit={midpoint_sha}\n"
        f"test_subject={midpoint_subject}\n"
        f"test_applied_commits={midpoint_position}\n"
        f"total_commits={count}\n"
    ...
    (ARTIFACTS / "linux-4.19.207-midpoint-state.txt").write_text(state)
    print(state, end="")

...
SECLOG_BYTES = 2 * 1024 * 1024
SECLOG_OFFSET_IN_11M = 2 * 1024 * 1024
HEADER_BYTES = 4096
RECORD_BYTES = 128
MAGIC = 0x313933474F4C4353
VERSION = 1
COMMIT = 0x391C0DE5
CAPACITY = (SECLOG_BYTES - HEADER_BYTES) // RECORD_BYTES
...
def choose_region(data: bytes) -> tuple[bytes, int]:
    if len(data) == SECLOG_BYTES:
        return data, 0
    if len(data) >= SECLOG_OFFSET_IN_11M + SECLOG_BYTES:
        region = data[
            SECLOG_OFFSET_IN_11M:
            SECLOG_OFFSET_IN_11M + SECLOG_BYTES
        ]
        return region, SECLOG_OFFSET_IN_11M
    raise SystemExit(
        f"expected a 2 MiB sec_log dump or a larger reserved-RAM dump; got {len(data)} bytes"
    )
...
def main() -> int:
    ...
    ap.add_argument("dump", type=Path,
                    help="2 MiB Phase391 sec_log dump or 11 MiB Samsung reserved-RAM dump")
    ...
    summary = {
        "input_bytes": len(raw),
        "sec_log_file_offset": file_offset,
    ...
    if magic != MAGIC or version != VERSION:
        ...
        raise SystemExit("Phase391 header not present")
    if record_bytes != RECORD_BYTES or capacity != CAPACITY:
        print(json.dumps(summary, indent=2, sort_keys=True))
        raise SystemExit("Phase391 geometry mismatch")

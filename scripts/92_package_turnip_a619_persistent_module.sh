...
version=0.20.4-vk1.4-pushdescriptor-validation
versionCode=30
author=touchGrass project
description=Persistent arm64 Mesa 26.2.2 Turnip Vulkan 1.4 capability-validation build for Adreno 619/KGSL. Driver is unchanged from the v0.20.1 golden baseline; the probe now executes core Vulkan 1.4 push descriptors plus maintenance6 vkCmdPushConstants2 on a real compute workload, alongside the passing host-image-copy round trip.
EOF
...
unzip -p "$ZIP" module.prop | grep -Fxq 'version=0.20.4-vk1.4-pushdescriptor-validation'

...
grep -Fq 'CONFIG_FUSE_PASSTHROUGH=y' "$KERNEL_DIR/arch/arm64/configs/a52xq_defconfig" || fail "Phase87 FUSE baseline missing before uplift"
grep -Fq 'update_segment_mtime(sbi, blkaddr);' "$KERNEL_DIR/fs/f2fs/segment.c" || fail "Phase88 F2FS age baseline missing before uplift"
grep -Fq 'f2fs_lookup_rb_tree_ext' "$KERNEL_DIR/fs/f2fs/extent_cache.c" || fail "Phase89 RB-tree baseline missing before uplift"
grep -Fq 'CURSEG_COLD_DATA_PINNED = NR_PERSISTENT_LOG' "$KERNEL_DIR/fs/f2fs/f2fs.h" || fail "Corrected in-memory curseg baseline missing before uplift"
...
test "$from_sha" = "b172b44fcb1771e083aad806fa96f3f60e2ddfac" || fail "Unexpected $FROM_TAG commit: $from_sha"
...
info "Three-way merging current Samsung/custom 4.19.206 tree with Linux $TARGET_VERSION"
...
rm -rf "$STABLE_DIR" "$BASE_TREE" "$THEIRS_TREE"
info "Phase91 Linux stable merge completed: $FROM_VERSION -> $TARGET_VERSION"

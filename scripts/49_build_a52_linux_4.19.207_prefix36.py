...
STABLE = WORKSPACE / "linux-stable-4.19.207-prefix36"
BASE_TREE = WORKSPACE / "linux-base-4.19.206-prefix36"
THEIRS_TREE = WORKSPACE / "linux-theirs-4.19.207-prefix36"
GOOD_TAG = "v4.19.206"
BAD_TAG = "v4.19.207"
...
def run(*args: str, cwd: Path | None = None, capture: bool = False) -> str:
    proc = subprocess.run(
        list(args), cwd=str(cwd) if cwd else None, check=True, text=True,
...
def main() -> None:
    ...
    run("git", "fetch", "--quiet", "--depth=1024", "origin", f"refs/tags/{BAD_TAG}:refs/tags/{BAD_TAG}", cwd=STABLE)
    ...
    commits = run(
        "git", "rev-list", "--reverse", "--first-parent", f"{GOOD_TAG}..{BAD_TAG}",
        cwd=STABLE, capture=True,
    ).splitlines()
    count = len(commits)
    if count < PRIOR_BAD_POSITION:
    ...
    prior_bad_subject = run("git", "show", "-s", "--format=%s", prior_bad_sha, cwd=STABLE, capture=True)
    test_subject = run("git", "show", "-s", "--format=%s", test_sha, cwd=STABLE, capture=True)

    commit_list = run(
        "git", "log", "--reverse", "--first-parent", "--format=%H%x09%s",
        f"{GOOD_TAG}..{BAD_TAG}", cwd=STABLE, capture=True,
    )
    ...
    status = ARTIFACTS / "linux-4.19.207-prefix36-name-status.zlist"
    with status.open("wb") as output:
        subprocess.run(
            ["git", "-C", str(STABLE), "diff", "--name-status", "-z", "--no-renames", GOOD_TAG, test_sha],
            stdout=output, check=True,
        )

    conflict_list = ARTIFACTS / "linux-4.19.207-prefix36-conflicts.txt"
    policy_report = ARTIFACTS / "linux-4.19.207-prefix36-policy.txt"
    policy_log = ARTIFACTS / "linux-4.19.207-prefix36-policy.tsv"
    run(
        "python3", str(POLICY), str(KERNEL), str(BASE_TREE), str(THEIRS_TREE),
        str(status), str(conflict_list), str(policy_report), str(policy_log), good_sha, test_sha,
    )

    candidate_version = kernel_version()
    generated_fix = ROOT / "scripts/checkpoint_fix_linux_4.19.207-prefix36.generated.sh"
    generated_fix.write_text(FIX_SCRIPT.read_text().replace(
    ...
    generated_build = ROOT / "scripts/build_a52_linux_4.19.207_prefix36.generated.sh"
    build_text = BUILD_TEMPLATE.read_text()
    build_text = build_text.replace("TARGET_VERSION=4.19.325", f"TARGET_VERSION={candidate_version}", 1)
    build_text = build_text.replace(
        "a52xq-p1-full-hardware-diag-", "a52xq-linux-4.19.207-prefix36-no-root-", 1
    )
    build_text = build_text.replace(
...
'''
    ...
    audit_symbols = [
        ...
        "DETECT_HUNG_TASK", "BOOTPARAM_HUNG_TASK_PANIC", "DEFAULT_HUNG_TASK_TIMEOUT",
        "PANIC_TIMEOUT",
    ]
    audit = []
    for symbol in audit_symbols:
        ...
        audit.append(next((line for line in final_config if line.startswith(prefix) or line == disabled), f"CONFIG_{symbol}=<absent>"))
    ...
    state = (
        ...
        "side_experiment_second_half_only=hardware-tested-good\n"
    )
    (out / "commit-bisect-state.txt").write_text(state)
    (ARTIFACTS / "linux-4.19.207-prefix36-state.txt").write_text(state)
    print(state, end="")

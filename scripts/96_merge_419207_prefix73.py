...
STABLE = WORKSPACE / "linux-stable-4.19.207-prefix73-current"
BASE_TREE = WORKSPACE / "linux-base-4.19.206-prefix73-current"
THEIRS_TREE = WORKSPACE / "linux-theirs-4.19.207-prefix73-current"
...
EXPECTED_TEST_SHA = "c5c62f4c936407fb734cae64700f20b68273b059"
EXPECTED_PRIOR_BAD_SHA = "c5c62f4c936407fb734cae64700f20b68273b059"
# Phase96 intentionally keeps boot diagnostics out of the bisect delta.
...
def main() -> None:
    ...
    if test_sha != EXPECTED_TEST_SHA:
        raise SystemExit(f"prefix73 SHA mismatch: {test_sha}")
    ...
    (ARTIFACTS / "phase96-linux-4.19.207-commit-list.tsv").write_text(commit_list + "\n")

    archive(STABLE, GOOD_TAG, BASE_TREE)
    archive(STABLE, test_sha, THEIRS_TREE)

    status = ARTIFACTS / "phase96-prefix73-name-status.zlist"
    ...
    conflicts = ARTIFACTS / "phase96-prefix73-conflicts.txt"
    report = ARTIFACTS / "phase96-prefix73-policy.txt"
    policy_log = ARTIFACTS / "phase96-prefix73-policy.tsv"
    run(
    ...
    generated_fix = ROOT / "scripts" / ".phase96-prefix73-compile-fix.sh"
    ...
    state = (
        "experiment=phase96-current-stack-419207-prefix73\n"
    ...
    (ARTIFACTS / "phase96-prefix73-state.txt").write_text(state)
    print(state, end="")

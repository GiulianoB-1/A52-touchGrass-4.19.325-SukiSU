#!/usr/bin/env python3
"""Phase 242 runtime identity for compact sticky CX state diagnostics."""
...
OLD = "BOOT rs=ready phase=241 focus=cx-broad-corridor-latch roots=%u copies=3 crc=crc32c"
NEW = "BOOT rs=ready phase=242 focus=cx-sticky-state roots=%u copies=3 crc=crc32c"
MARKER = "A52_PHASE242_CX_STICKY_STATE_IDENTITY_V1"
IDENTITY_ANCHOR = "\t * A52_PHASE242_CX_STICKY_STATE_V1\n"
...
def patch(text: str, label: str) -> str:
    if NEW in text and MARKER in text:
        return text
    if text.count(OLD) != 1:
        raise RuntimeError(f"{label}: expected exactly one Phase 241 boot identity, found {text.count(OLD)}")
    text = text.replace(OLD, NEW, 1)
    if text.count(IDENTITY_ANCHOR) != 1:
        raise RuntimeError(f"{label}: expected exactly one Phase 242 sticky-state anchor, found {text.count(IDENTITY_ANCHOR)}")
...
def locate_generated(args: list[str], cwd: Path | None = None) -> Path:
    ...
    if len(unique) != 1:
        ...
        raise RuntimeError(f"expected one generated Phase 242 recorder root, found {rendered}")
    return unique[0]
...
def self_test() -> None:
    ...
    print("Phase 242 runtime identity self-test: PASS", flush=True)
...
def main() -> int:
    ...
    print("Phase 242 runtime identity applied", flush=True)
    return 0

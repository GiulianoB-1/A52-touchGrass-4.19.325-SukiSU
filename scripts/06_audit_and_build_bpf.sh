...
AUDIT_FLAGS="-Werror=incompatible-pointer-types -Werror=implicit-function-declaration -Werror=return-type"
...
info "Compiling kernel/bpf/verifier.o with type warnings treated as errors"
set +e
{
  make -C "$KERNEL_DIR" O="$AUDIT_OUT" \
    DTC_EXT="$KERNEL_DIR/tools/dtc" \
...
warning_count=$(grep -c 'warning:' "$AUDIT_LOG" || true)
incompatible_count=$(grep -Eci 'incompatible pointer|incompatible-pointer-types' "$AUDIT_LOG" || true)
implicit_count=$(grep -Eci 'implicit function|implicit-function-declaration' "$AUDIT_LOG" || true)

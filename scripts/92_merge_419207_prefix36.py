...
STABLE = WORKSPACE / "linux-stable-4.19.207-prefix36-current"
BASE_TREE = WORKSPACE / "linux-base-4.19.206-prefix36-current"
THEIRS_TREE = WORKSPACE / "linux-theirs-4.19.207-prefix36-current"
...
def main() -> None:
    ...
    if test_sha != EXPECTED_TEST_SHA:
        raise SystemExit(f"prefix36 SHA mismatch: {test_sha}")
    ...
    (ARTIFACTS / "phase92-linux-4.19.207-commit-list.tsv").write_text(commit_list + "\n")

    archive(STABLE, GOOD_TAG, BASE_TREE)
    archive(STABLE, test_sha, THEIRS_TREE)

    status = ARTIFACTS / "phase92-prefix36-name-status.zlist"
    ...
    conflicts = ARTIFACTS / "phase92-prefix36-conflicts.txt"
    report = ARTIFACTS / "phase92-prefix36-policy.txt"
    policy_log = ARTIFACTS / "phase92-prefix36-policy.tsv"
    run(
    ...
    generated_fix = ROOT / "scripts" / ".phase92-prefix36-compile-fix.sh"
    ...
    state = (
        "experiment=phase92-current-stack-419207-prefix36\n"
    ...
    (ARTIFACTS / "phase92-prefix36-state.txt").write_text(state)
    print(state, end="")

...
TARGET_COMMIT=7c6679265082335bca08b954208b385143c0d4de
...
# Keep tightly coupled generic subsystems internally consistent. Device-specific
# Samsung and Qualcomm drivers remain vendor-preserved by the merge policy.
...
info "Repairing merge-shape compatibility issues"
python3 - \
...
if menus == 1 and endmenus == 0:
    ...
    repairs.append("drivers/char/Kconfig=appended-missing-endmenu")
elif menus != endmenus:
    raise SystemExit(
...
if count == 2:
    ...
    repairs.append("arch/arm64/include/asm/sections.h=removed-duplicate-entry-tramp-helper")
elif count not in (0, 1):
    raise SystemExit(f"unexpected entry_tramp_text_size definition count: {count}")
...
git -C "$KERNEL_DIR" diff --check > "$ARTIFACTS_DIR/direct-merge-diff-check-$TO_TAG.txt" 2>&1 || true
rm -rf "$STABLE_DIR" "$BASE_TREE" "$THEIRS_TREE"

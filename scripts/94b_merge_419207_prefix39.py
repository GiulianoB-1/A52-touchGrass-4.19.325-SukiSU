...
STABLE = WORKSPACE / "linux-stable-4.19.207-prefix39-current"
BASE_TREE = WORKSPACE / "linux-base-4.19.206-prefix39-current"
THEIRS_TREE = WORKSPACE / "linux-theirs-4.19.207-prefix39-current"
...
EXPECTED_TEST_SHA = "e6c3fefc6bb11bef1bd8adfe37e0f317303ab751"
EXPECTED_PRIOR_BAD_SHA = "c5c62f4c936407fb734cae64700f20b68273b059"
# Phase94B intentionally keeps boot diagnostics out of the bisect delta.
...
def main() -> None:
    ...
    if test_sha != EXPECTED_TEST_SHA:
        raise SystemExit(f"prefix39 SHA mismatch: {test_sha}")
    ...
    (ARTIFACTS / "phase94b-linux-4.19.207-commit-list.tsv").write_text(commit_list + "\n")

    archive(STABLE, GOOD_TAG, BASE_TREE)
    archive(STABLE, test_sha, THEIRS_TREE)

    status = ARTIFACTS / "phase94b-prefix39-name-status.zlist"
    ...
    conflicts = ARTIFACTS / "phase94b-prefix39-conflicts.txt"
    report = ARTIFACTS / "phase94b-prefix39-policy.txt"
    policy_log = ARTIFACTS / "phase94b-prefix39-policy.tsv"
    run(
    ...
    generated_fix = ROOT / "scripts" / ".phase94b-prefix39-compile-fix.sh"
    ...
    state = (
        "experiment=phase94b-current-stack-419207-prefix39\n"
    ...
    (ARTIFACTS / "phase94b-prefix39-state.txt").write_text(state)
    print(state, end="")

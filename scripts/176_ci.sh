...
OUT="$PWD/artifacts/a52xq-display-recorder-fec-prz-v2"
BUILD="$PWD/workspace/gki-display-recorder-fec-prz-v2-out"
...
# Keep the matching decoder from the packed payload, but restore the committed
# PRZ-mapping patcher after the legacy payload extracts its v1 copy.
git checkout -- scripts/176_apply_a52_recorder_fec.py
...
grep -Fq '"persistent_profile": "display-bindcore-fec-prz-v2"' "$REPORT"
grep -Fq '"mapping_backend": "persistent_ram_new-vmap-owned-fixed-slots"' "$REPORT"
...
grep -Fq 'A52_REC3_PROFILE "display-bindcore-fec-prz-v2"' "$REC"
...
git -C gki/common diff --binary --no-ext-diff \
  > "$OUT/stage/display-recorder-fec-prz-v2-source.patch"
test -s "$OUT/stage/display-recorder-fec-prz-v2-source.patch"
...
for marker in \
  'display-bindcore-fec-prz-v2' \
  'BOOT recorder=v3 profile=%s copies=3 rs=32 crc32c=1 slots=1023' \
  'A52 recorder v3 PRZ-mapped %u banks, slots=%u, RS parity=%u' \
...
root = Path('artifacts/a52xq-display-recorder-fec-prz-v2')
stage = json.loads((root / 'stage/phase34-a52-recorder-fec-report.json').read_text())
repack = json.loads((root / 'package/repack-report.json').read_text())
image = root / 'compile/Image'
boot = root / 'package/boot.img'
audit = {
    'status': 'a52-display-recorder-fec-prz-v2-boot-audited',
    'flashable_candidate': True,
    'hardware_validated': False,
    'persistent_profile': 'display-bindcore-fec-prz-v2',
...
A52 display recorder FEC PRZ-mapping candidate

This revision preserves the exact recorder-v3 256-byte format, CRC32C,
32-byte Reed-Solomon parity, commit footer, and three fixed-slot copies.

The only recorder change is the RAM mapping path. Each bank is now mapped by
persistent_ram_new(), allowing the kernel to use vmap() for the A52's valid
reserved-RAM PFNs. The recorder then writes its existing fixed-slot format
through the mapping owned by the persistent_ram_zone.

...
LABEL="a52xq-linux-4.19.206-touchscreen-no-root"
OUT="$ARTIFACTS_DIR/a52xq-linux-4.19.206-touchscreen-kernel"
...
# Keep the complete A52/Lagoon hardware configuration and explicitly restore the
# active STMicroelectronics FTS touchscreen driver. Root integration remains off.
...
required_y=(
  ...
  TOUCHSCREEN_STM_FTS5CU56A INPUT_QPNP_POWER_ON DRM QCOM_KGSL MEDIA_SUPPORT
  SOUND SND WLAN CFG80211 MAC80211 BT PRINTK PRINTK_TIME DEBUG_KERNEL
  FRAME_POINTER PANIC_ON_OOPS PSTORE PSTORE_RAM PSTORE_CONSOLE PSTORE_PMSG
  SERIAL_MSM_GENI SERIAL_MSM_GENI_CONSOLE
)
...
[[ "$missing" == 0 ]] || fail "One or more required built-ins were not retained"

! grep -Eq '^CONFIG_KSU(=|_)' "$FINAL_CONFIG" || fail "KernelSU integration remained enabled"
grep -Fxq 'CONFIG_TOUCHSCREEN_STM_FTS5CU56A=y' "$FINAL_CONFIG" \
  || fail "STM FTS touchscreen driver was not built in"

{
  echo "active_touch_driver=stm,fts_touch"
  echo "enabled_symbol=CONFIG_TOUCHSCREEN_STM_FTS5CU56A"
  grep -E '^(CONFIG_INPUT_TOUCHSCREEN=|CONFIG_TOUCHSCREEN_STM_FTS5CU56A=)' "$FINAL_CONFIG"
...

{
  ...
  echo "touch_policy=enable-stm-fts"
  echo "root_integration=none"
...
NON-FLASHABLE KERNEL BUILD ARTIFACT.

Linux 4.19.206 with the A52 STMicroelectronics FTS touchscreen driver enabled.
Root integration is disabled. This kernel must be repacked into the locked boot
image and audited before physical use.
EOF

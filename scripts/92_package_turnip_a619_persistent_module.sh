...
version=0.20.2-vk1.4-capability-validation
versionCode=28
author=touchGrass project
description=Persistent arm64 Mesa 26.2.2 Turnip Vulkan 1.4 capability-validation build for Adreno 619/KGSL. Driver is unchanged from the v0.20.1 golden baseline; the probe now enables the reported Vulkan 1.4 feature block and audits representative core 1.4 dispatch paths.
EOF
...
unzip -p "$ZIP" module.prop | grep -Fxq 'version=0.20.2-vk1.4-capability-validation'

...
CXF241 must be admitted by the actual post-capacity critical classifier, and
source-side CXF241 create/dreg evidence must remain classifiable while the
independent replay-recursion guard remains present.
"""
...
BLANKET_EXCLUSION = 'if (!message || !strncmp(message, "CXF241 ", 7))'
...
def audit_recorder(text: str, label: str) -> None:
    require(text, (
        IDENTITY, IDENTITY_MARKER, LATCH_MARKER, RETENTION_MARKER, CLASSIFY_MARKER,
    ...
    critical_count = text.count('!strncmp(message, "CXF241 ", 7)')
    if critical_count != 1:
        raise RuntimeError(
            f"{label}: expected exactly one CXF241 critical prefix, found {critical_count}"
        )
    if BLANKET_EXCLUSION in text:
        raise RuntimeError(
            f"{label}: blanket CXF241 classifier exclusion remains; create/dreg branches are unreachable"
        )
...
def locate_generated(args: list[str], cwd: Path | None = None) -> Path:
    ...
    for root in candidate_roots(args, base):
        ...
        if LATCH_MARKER not in text or IDENTITY_MARKER not in text or RETENTION_MARKER not in text or CLASSIFY_MARKER not in text: continue
        key = root.resolve()
...
def main() -> int:
    ...
    print("Phase 241 generated-source audit: PASS (HB/live/replay same function; source create/dreg classifiable; CXF241 post-capacity critical)", flush=True)
    return 0

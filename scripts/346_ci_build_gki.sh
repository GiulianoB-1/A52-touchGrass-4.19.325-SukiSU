...
fail_report() {
  ...
  cp phase346-*.log "$FAIL/logs/" 2>/dev/null || true
  cp /tmp/p346-* "$FAIL/audit/" 2>/dev/null || true
  cp scripts/346_apply_dma_raw.py scripts/346_ci_build_gki.sh "$FAIL/audit/" 2>/dev/null || true
...
if "\ta52_r342_start();" in rec:
    raise SystemExit("Phase346: Phase342 runtime writer is active")
print("Phase346 Phase342-sideband ownership audit: PASS")
PY
...
bh = Path("/tmp/p346-hwc-before").read_text()
bc = Path("/tmp/p346-ctrl-before").read_text()
...
for token in (
    ...
    if token not in h + c:
        raise SystemExit("Phase346 missing " + token)

for token in (
    "DSI_W32(", "DSI_R32(", "wait_for_completion_timeout(",
    "clk_set_rate(", "regulator_enable(", "reset_control_",
    "udelay(", "usleep_range(", "msleep(",
):
    if h.count(token) != bh.count(token) or c.count(token) != bc.count(token):
        raise SystemExit("Phase346 changed measured primitive " + token)
...
make -C "$ROOT" O="$BUILD" \
  ...
  LLVM=1 LLVM_IAS=1 \
  olddefconfig > phase346-olddefconfig.log 2>&1
...
make -C "$ROOT" O="$BUILD" \
  ...
  CLANG_TRIPLE=aarch64-linux-gnu- \
  LLVM=1 LLVM_IAS=1 \
  -j"$(nproc)" Image 2>&1 | tee phase346-compile.log
rc=${PIPESTATUS[0]}
set -e
test "$rc" -eq 0
...
# a52_p346_sideband_init() is externally referenced by dsi_ctrl.c, so this
# symbol must survive optimization. The static persistence helper may inline.
...
=========================
Physical region: 0xB1BFA000..0xB1BFC000
Former Phase342 sideband; Phase342 runtime writer is disabled by Phase343.
...
magic   = 0x3634335741524d44
commit  = 0x346c0de5
version = 1

Samples are copied to the sideband only after p6 and after DEBUG_BUS_CTL is
restored, so the p0..p6 measurement burst itself is unchanged from Phase345.
EOF
...
identity = {
    ...
    "mirrored": True,
    "slot_bytes": 96,
    "phase342_runtime_writer_disabled": True,
    "measured_dsi_burst_changed": False,

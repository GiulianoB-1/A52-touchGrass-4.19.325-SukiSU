...
for f,toks in {
 ...
 'source/dsi_ctrl.c':['A52_PHASE276R_DSI_CTRL_DEEP_FRONTIER_V1','P276 D C s=0 f=%x','P276 D M s=0 f=%x mt=%u l=%u','P276 D K s=0 f=%x','P276 D W s=0','P276 D M w=0','P276 D M w=1 v=%d'],
}.items():
    s=(r/f).read_text()
    for t in toks:
        if t not in s: raise SystemExit(f'{f}: missing {t}')
img=(r/'compile/Image').read_bytes()
for t in ['P276 T Q p=%d g=%d y=%d','P276 D H s=0','P276 D C s=0 f=%x','P276 D M s=0 f=%x mt=%u l=%u','P276 D K s=0 f=%x','P276 D W s=0','P276 D M w=0','P276 D M w=1 v=%d','P276 T O i=%d p=0 mt=%u tl=%u fl=%x']:
...
for t in ['function=dsi_host_transfer','function=dsi_ctrl_cmd_transfer','function=dsi_message_tx','function=dsi_kickoff_msg_tx','function=dsi_ctrl_dma_cmd_wait_for_done','all_exact_match=1']:

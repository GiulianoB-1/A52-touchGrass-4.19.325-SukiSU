...
GENERATED="$SCRIPT_DIR/.phase102-generated-resukisu-susfs.sh"
...
: "${LABEL:?Phase102 LABEL environment variable is required}"

if [ "$TARGET_VERSION" != "4.19.236" ]; then
  echo "Expected Phase102 candidate 4.19.236, got $TARGET_VERSION" >&2
  exit 1
fi

# Phase102 runs only after the prepared Phase88 tree has been validated and,
# on a cache miss, quiesced so the complete Git object database is safe to archive.
# Repair only the two 4.19.236 vendor/upstream merge shapes discovered by the
# first full compile: pm_show_wakelocks() and the FUSE private inode-state bit.
bash "$SCRIPT_DIR/102_fix_419236_compile.sh"
...
echo "phase102_kernelversion=$TARGET_VERSION"
echo "phase102_image=$CANON_IMAGE"
echo "phase102_config=$CANON_CONFIG"

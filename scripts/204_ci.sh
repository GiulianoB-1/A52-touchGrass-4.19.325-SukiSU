...
cp "$ROOT/drivers/firmware/qcom_scm.c" \
  "$OUT/comparison/gki-qcom_scm.c"
cp "$ROOT/include/linux/qcom_scm.h" \
  "$OUT/comparison/gki-qcom_scm.h"
cp "$TG/drivers/iommu/arm-smmu.c" \
  "$OUT/comparison/touchgrass-arm-smmu.c"
cp "$TG/drivers/soc/qcom/scm.c" \
  "$OUT/comparison/touchgrass-direct-scm.c"
cp "$TG/include/soc/qcom/scm.h" \
  "$OUT/comparison/touchgrass-direct-scm.h"
cp "$TG/arch/arm64/boot/dts/vendor/qcom/lagoon.dtsi" \
  "$OUT/comparison/touchgrass-lagoon.dtsi"
...
(out / 'touchgrass-comparison.json').write_text(
    json.dumps(report, indent=2, sort_keys=True) + '\n'
)
print(json.dumps(report, indent=2, sort_keys=True))
PY

python3 scripts/204_apply_apps_smmu_scm_handoff.py \
...
cmp "$OUT/stage/arm-smmu-before-phase204.c" \
  "$OUT/stage/arm-smmu-after-phase204.c"
...
git -C "$ROOT" diff --binary --no-ext-diff -- \
  ...
  > "$OUT/stage/phase204-apps-smmu-scm-handoff.patch"
...
TouchGrass-first comparison result:
  ...
  - TouchGrass SMMU has no such readiness gate and uses its separate downstream
    direct-SMC implementation for secure operations
  - in the GKI Lagoon skip-init path, the Qualcomm reset hook is suppressed and
    no SCM operation is issued by arm-smmu-qcom

Phase 204 changes only the Qualcomm implementation readiness condition. When the
node is exactly qcom,qsmmu-v500 and also declares qcom,skip-init, it may preserve
the bootloader handoff and continue without the absent upstream qcom_scm platform
device. Every other Qualcomm SMMU configuration retains the original SCM gate.

No new recorder is added. Existing Phase 203 checkpoints remain available. No
forced bind, device_attach call, IOMMU bypass, supplier relaxation, DTB change,
DTBO change, ramdisk change, panel command change, timing change, or power-policy
change is included.
...
base.update({
    'status': 'a52-apps-smmu-scm-handoff-audited',
    'phase': 204,
    'base_phase': 203,
    ...
    'qcom_scm_driver_modified': False,
    'dtb_changed': False,
    'dtbo_changed': False,
...
for key in (
    ...
    'touchgrass_compared_before_fix',
    'qsmmuv500_skip_init_handoff_added',
    'generic_qcom_scm_gate_preserved',
...
for key in ('new_recorder_added', 'forced_bind_added', 'device_attach_added',
            'iommu_bypass_added', 'supplier_dependency_relaxed',
            'qcom_scm_driver_modified'):

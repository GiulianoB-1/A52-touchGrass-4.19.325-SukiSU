...
version=0.20.3-vk1.4-hostcopy-validation
versionCode=29
author=touchGrass project
description=Persistent arm64 Mesa 26.2.2 Turnip Vulkan 1.4 capability-validation build for Adreno 619/KGSL. Driver is unchanged from the v0.20.1 golden baseline; the probe executes a core Vulkan 1.4 host-image-copy CPU-to-image-to-CPU round trip in addition to feature-enable and dispatch validation.
EOF
...
unzip -p "$ZIP" module.prop | grep -Fxq 'version=0.20.3-vk1.4-hostcopy-validation'

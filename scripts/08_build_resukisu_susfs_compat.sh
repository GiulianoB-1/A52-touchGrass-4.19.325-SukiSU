...
config_anchor = '    text = text.replace(old_config, new_config, 1)\n'
config_injection = config_anchor + r'''
    minimal_extra = (' \\\n'
                     ...
                     '  -d KSU_SUSFS_SUS_MAP')
    if text.count(new_config) != 1:
        raise SystemExit('generated SUSFS config block mismatch')
    text = text.replace(new_config, new_config + minimal_extra, 1)
'''
if text.count(config_anchor) != 1:
    raise SystemExit('checkpoint config injection anchor mismatch')
text = text.replace(config_anchor, config_injection, 1)
...
text = text.replace("require_line \"$FINAL_CONFIG\" '# CONFIG_KSU_SUSFS is not set'", "require_line \"$FINAL_CONFIG\" 'CONFIG_KSU_SUSFS=y'")
text = text.replace("printf 'resukisu_version=%s\\n' \"$RESUKISU_VERSION_FULL\"", "printf 'resukisu_version=%s\\n' \"$RESUKISU_VERSION_FULL\"\n  printf 'susfs_version=%s\\n' \"$SUSFS_VERSION\"\n  printf 'susfs_profile=minimal-root-module-test\\n'")

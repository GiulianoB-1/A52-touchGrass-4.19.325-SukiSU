...
fail_report() {
  ...
  cp scripts/319_reconstruct_phase206_from_verified_artifact.sh "$FAIL/audit/" 2>/dev/null || true
  [ -f "$P316/BUILD-IDENTITY.json" ] && cp "$P316/BUILD-IDENTITY.json" "$FAIL/audit/PHASE316-ORACLE-BUILD-IDENTITY.json" || true
...
stage "install verified Phase206 reconstruction bridge"
python3 -m py_compile scripts/319_apply_dsi_sixpoint_temporal_observer.py
bash -n scripts/319_reconstruct_phase206_from_verified_artifact.sh
cp scripts/208_reconstruct_phase206_source.sh /tmp/p319gki-208-original.sh
cp scripts/319_reconstruct_phase206_from_verified_artifact.sh scripts/208_reconstruct_phase206_source.sh
cmp -s scripts/208_reconstruct_phase206_source.sh scripts/319_reconstruct_phase206_from_verified_artifact.sh

stage "reconstruct full exact Phase316 lineage"
set +e
bash scripts/316_ci_build.sh 2>&1 | tee phase319-gki-phase316.log
rc=${PIPESTATUS[0]}
set -e
cp /tmp/p319gki-208-original.sh scripts/208_reconstruct_phase206_source.sh
test "$rc" -eq 0
for f in \
  phase316-gki-out/package/boot.img \
  phase316-gki-out/compile/Image \
  phase316-gki-out/config/final.config \
  "$CTRL" "$HWC" "$PHY" "$PHYV3" "$DISP"; do test -s "$f"; done
...
stage "strict Phase319 scope audit"
python3 - <<'PY'
from pathlib import Path
...
python3 scripts/38_repack_a52_p1_boot.py \
  --source phase316-gki-out/package/boot.img \

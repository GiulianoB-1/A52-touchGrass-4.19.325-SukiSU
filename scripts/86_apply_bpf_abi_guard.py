...
if len(sys.argv) != 2:
    raise SystemExit("usage: 86_apply_bpf_abi_guard.py <kernel-tree>")
...
s = filt.read_text()
marker = "A52_PHASE86_BPF_ABI_GUARD_V1"
if marker not in s:
...
if failed:
    raise SystemExit("phase86 audit failed: " + ", ".join(failed))

print("phase86_bpf_abi_guard=applied")

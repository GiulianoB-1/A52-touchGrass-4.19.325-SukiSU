...
BUILD="$PWD/workspace/gki-display-recorder-fec-out"
...
grep -Fq '"persistent_profile": "display-bindcore-fec-v1"' "$REPORT"
...
grep -Fq 'A52_REC3_PROFILE "display-bindcore-fec-v1"' "$REC"
...
grep -Fq 'init_rs(8, 0x11d, 0, 1, A52_ACKFR_PARITY_BYTES)' "$RAM"
...
git -C gki/common diff --binary --no-ext-diff \
  > "$OUT/stage/display-recorder-fec-source.patch"
test -s "$OUT/stage/display-recorder-fec-source.patch"
...
for marker in \
  'display-bindcore-fec-v1' \
  'BOOT recorder=v3 profile=%s copies=3 rs=32 crc32c=1 slots=1023' \
  'A52 recorder v3 mapped %u banks, slots=%u, RS parity=%u' \
...
root = Path('artifacts/a52xq-display-recorder-fec')
stage = json.loads((root / 'stage/phase34-a52-recorder-fec-report.json').read_text())
repack = json.loads((root / 'package/repack-report.json').read_text())
image = root / 'compile/Image'
boot = root / 'package/boot.img'
audit = {
    'status': 'a52-display-recorder-fec-boot-audited',
    'flashable_candidate': True,
    'hardware_validated': False,
    'persistent_profile': 'display-bindcore-fec-v1',
...
A52 display recorder FEC candidate

This candidate preserves the proven heap-19, QSEECOM, REFGEN, and display-
binding audit source. The functional change is limited to persistent diagnostic
recording.

Recorder v3 stores only BOOT, HB, REFGEN, DISP, and WDT metadata. Every
256-byte event has CRC32C, 32 Reed-Solomon parity bytes, a final commit footer,
and three copies in the record, console, and ftrace quarters of the reserved
1 MiB RAMOOPS range.

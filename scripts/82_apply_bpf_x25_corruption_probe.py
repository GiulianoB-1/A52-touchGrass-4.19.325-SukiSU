...
if len(sys.argv) != 2:
    raise SystemExit("usage: 82_apply_bpf_x25_corruption_probe.py <kernel-tree>")
...
/*
 * Phase82: detect ARM64 BPF JIT x25 ABI corruption while preserving
 * the caller's x25 so the diagnostic kernel can continue booting.
 */
...
ENTRY(a52_bpf_call_x25_probe)
	...
	/* Capture the JIT-returned x25 before repairing it. */
	ldr	x3, [sp, #24]
	str	x25, [x3, #8]
	ldr	x16, [sp, #16]
	str	x16, [x3, #0]
...
h = hdr.read_text()
marker = "A52_PHASE82_BPF_X25_CORRUPTION_PROBE_V1"
if marker not in h:
...
checks = {
    ...
    "asm_capture_after": "str\tx25, [x3, #8]" in asm.read_text(),
    "asm_restore_x25": "mov\tx25, x16" in asm.read_text(),
...
print("phase82_bpf_x25_corruption_probe=applied")

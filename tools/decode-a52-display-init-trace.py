...
def output(
    ...
    with (root / "display-init-events.csv").open("w", newline="", encoding="utf-8") as handle:
    ...
    with (root / "display-init-events.txt").open("w", encoding="utf-8") as handle:
    ...
    summary = {
        ...
        "display": {
            "events": sum(domain(event.message) == "DISP" for event in events),
            "first": next((event.message for event in events if domain(event.message) == "DISP"), None),
            "last": next((event.message for event in reversed(events) if domain(event.message) == "DISP"), None),
        },
        "error_correction": False,
        "binary_record_format": False,
        "privacy": (
            "plain-text metadata trace; includes DSI command type, length, "
            "and bounded payload prefix/suffix but no process memory, keys, or tokens"
        ),
    }
...
def self_test() -> None:
    lines = (
        ...
        b"commit=5a52c0de\n"
        b"A52USR2 seq=3 ns=300 pid=1 tgid=1 cpu=0 comm=init "
        b"msg=DISP DSI i=0 t=39 c=0 f=0 n=2 r=0 h=11aa000000000000 z=000011aa "
        b"commit=5a52c0de\n"
    )
    ...
    if domain(events[2].message) != "DISP":
        raise SystemExit("decoder display-domain self-test failed")


def main() -> int:
    parser = argparse.ArgumentParser(
        description="Decode the plain-text A52 display-initialization RAMOOPS trace"
    )
    parser.add_argument("inputs", nargs="*", type=Path)
    parser.add_argument(
        "--output", type=Path, default=Path("a52-display-init-decoded")
    )
    parser.add_argument("--self-test", action="store_true")
    args = parser.parse_args()
    self_test()
    if args.self_test:
        print(json.dumps({"status": "self-test-passed", "error_correction": False}, sort_keys=True))
        return 0
    if not args.inputs:

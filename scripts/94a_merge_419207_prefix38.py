...
STABLE = WORKSPACE / "linux-stable-4.19.207-prefix38-current"
BASE_TREE = WORKSPACE / "linux-base-4.19.206-prefix38-current"
THEIRS_TREE = WORKSPACE / "linux-theirs-4.19.207-prefix38-current"
...
EXPECTED_TEST_SHA = "293fe77dbfe68c5794be4e76c9212003e9304d24"
EXPECTED_PRIOR_BAD_SHA = "c5c62f4c936407fb734cae64700f20b68273b059"
# Phase94A intentionally keeps boot diagnostics out of the bisect delta.
...
def main() -> None:
    ...
    if len(commits) < PRIOR_BAD_POSITION:
    ...
    if test_sha != EXPECTED_TEST_SHA:
        raise SystemExit(f"prefix38 SHA mismatch: {test_sha}")
    if prior_bad_sha != EXPECTED_PRIOR_BAD_SHA:
        raise SystemExit(f"prefix73 SHA mismatch: {prior_bad_sha}")
    ...
    (ARTIFACTS / "phase94a-linux-4.19.207-commit-list.tsv").write_text(commit_list + "\n")

    archive(STABLE, GOOD_TAG, BASE_TREE)
    archive(STABLE, test_sha, THEIRS_TREE)

    status = ARTIFACTS / "phase94a-prefix38-name-status.zlist"
    ...
    conflicts = ARTIFACTS / "phase94a-prefix38-conflicts.txt"
    report = ARTIFACTS / "phase94a-prefix38-policy.txt"
    policy_log = ARTIFACTS / "phase94a-prefix38-policy.tsv"
    run(
    ...
    generated_fix = ROOT / "scripts" / ".phase94a-prefix38-compile-fix.sh"
    ...
    state = (
        "experiment=phase94a-current-stack-419207-prefix38\n"
    ...
    (ARTIFACTS / "phase94a-prefix38-state.txt").write_text(state)
    print(state, end="")

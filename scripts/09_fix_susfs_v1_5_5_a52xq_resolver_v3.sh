...
cat > "$REPLACEMENT" <<'NAMESPACE_BLOCK'
python3 - "$KERNEL_DIR/fs/namespace.c" <<'PY_NAMESPACE'
...
def replace_in_function(source: str, function_marker: str, next_marker: str,
...
parent_lock_old = '''	mnt->mnt_parent = mnt;
	lock_mount_hash();
'''
vfs_parent_lock_new = '''	mnt->mnt_parent = mnt;
#ifdef CONFIG_KSU_SUSFS_SUS_MOUNT
	...
	lock_mount_hash();
'''
text = replace_in_function(text,
                           'vfs_kern_mount(struct file_system_type *type',
                           'static struct mount *clone_mnt(',
                           parent_lock_old, vfs_parent_lock_new,
                           "vfs_kern_mount fake mount ID")

clone_parent_lock_new = '''	mnt->mnt_parent = mnt;
#ifdef CONFIG_KSU_SUSFS_SUS_MOUNT
...
text = replace_in_function(text,
                           ...
                           parent_lock_old, clone_parent_lock_new,
...
# Keep the generated unified patch as an auditable description.
sha256sum "$A52_NAMESPACE_PATCH" > "$A52_NAMESPACE_PATCH.sha256"
NAMESPACE_BLOCK

python3 - "$TARGET" "$REPLACEMENT" <<'PY_PATCHER'
from pathlib import Path
import sys

path = Path(sys.argv[1])
replacement = Path(sys.argv[2]).read_text().rstrip() + "\n\n"
text = path.read_text()

start_marker = 'git -C "$KERNEL_DIR" apply --check --whitespace=nowarn "$A52_NAMESPACE_PATCH"'
end_marker = 'python3 - "$KERNEL_DIR/include/linux/sched.h" "$KERNEL_DIR/kernel/sys.c" <<\'PY_A52\''

start = text.find(start_marker)
if start < 0:
    raise SystemExit("namespace resolver start marker not found")
end = text.find(end_marker, start)
if end < 0:
    raise SystemExit("namespace resolver end marker not found")
if text.find(start_marker, start + 1) >= 0:
    raise SystemExit("namespace resolver start marker is not unique")

path.write_text(text[:start] + replacement + text[end:])
PY_PATCHER

bash -n "$TARGET"
echo "Made A52XQ SUSFS namespace resolution context-safe without nested quoting"
